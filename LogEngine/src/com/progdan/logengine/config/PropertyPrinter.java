...
package com.progdan.logengine.config;
...
/**
   Prints the configuration of the logengine default hierarchy
...
public class PropertyPrinter implements PropertyGetter.PropertyCallback {
  ...
  */
  ...
  /**
   * Prints the configuration of the default logengine hierarchy as a Java
  ...
  void printOptions(PrintWriter out, Logger cat) {
    ...
    while (appenders.hasMoreElements()) {
      ...
      if ((name = (String) appenderNames.get(app)) == null) {
        ...
        printOptions(out, app, "logengine.appender."+name);
        if (app.getLayout() != null) {
          printOptions(out, app.getLayout(), "logengine.appender."+name+".layout");
        }
      }
      appenderString += ", " + name;
    }
    String catKey = (cat == Logger.getRootLogger())
        ? "logengine.rootLogger"
        : "logengine.logger." + cat.getName();
    if (appenderString != "") {
