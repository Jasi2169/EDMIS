...
import com.progdan.checksum.util.GeneralProgram;
...
public class ChecksumAPI {
     ...
     * always try to use fast implementations from the Java API
     ...
     * get an object of a checksum algoritm
     * @param algorithm code for the checksum algorithm
     * @param alternate use a pure Java implementation (only if available)
     * @return a checksum algorithm object, returns null if algorithm is unknown
     */
    ...
    throws NoSuchAlgorithmException {
        ...
        if (algorithm.equals("sha") || algorithm.equals("sha1") || algorithm.equals("sha-1") ||
            algorithm.equals("sha160") || algorithm.equals("sha-160")) {
            if (alternate) checksum = new MDgnu(com.progdan.checksum.crypto.Registry.SHA160_HASH); else
            ...
            if (alternate) checksum = new MDgnu(com.progdan.checksum.crypto.Registry.MD5_HASH); else
            checksum = new MD("MD5");
        ...
        /* we use faster versions (supported since 1.4.2) if possible
         ...
         */
            ...
            if (alternate) checksum = new MDgnu(com.progdan.checksum.crypto.Registry.SHA256_HASH); else
            if (GeneralProgram.isSupportFor("1.4.2"))
                checksum = new MD("SHA-256"); else
                checksum = new MDgnu(com.progdan.checksum.crypto.Registry.SHA256_HASH);
        } else if (algorithm.equals("sha384") || algorithm.equals("sha-384")) {
            if (alternate) checksum = new MDgnu(com.progdan.checksum.crypto.Registry.SHA384_HASH); else
            if (GeneralProgram.isSupportFor("1.4.2"))
                checksum = new MD("SHA-384"); else
                checksum = new MDgnu(com.progdan.checksum.crypto.Registry.SHA384_HASH);
        } else if (algorithm.equals("sha512") || algorithm.equals("sha-512")) {
            if (alternate) checksum = new MDgnu(com.progdan.checksum.crypto.Registry.SHA512_HASH); else
            if (GeneralProgram.isSupportFor("1.4.2"))
                checksum = new MD("SHA-512"); else
                checksum = new MDgnu(com.progdan.checksum.crypto.Registry.SHA512_HASH);
        } else if (algorithm.equals("tiger")) {
            checksum = new MDgnu(com.progdan.checksum.crypto.Registry.TIGER_HASH);
            ...
            checksum = new MDgnu(com.progdan.checksum.crypto.Registry.RIPEMD160_HASH);
            ...
            checksum = new MDgnu(com.progdan.checksum.crypto.Registry.RIPEMD128_HASH);
        } else if (algorithm.equals("whirlpool")) {
            checksum = new MDgnu(com.progdan.checksum.crypto.Registry.WHIRLPOOL_HASH);
            ...
            checksum = new MDgnu(com.progdan.checksum.crypto.Registry.MD4_HASH);
        } else if (algorithm.equals("md2") || algorithm.equals("md2sum")) {
            checksum = new MDgnu(com.progdan.checksum.crypto.Registry.MD2_HASH);
