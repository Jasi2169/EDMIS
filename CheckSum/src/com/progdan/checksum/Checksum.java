package com.progdan.checksum;

import com.progdan.checksum.algorithm.*;
import com.progdan.checksum.util.*;
import java.io.*;
import java.util.*;
import java.text.*;
import com.progdan.checksum.crypto.*;
import com.progdan.checksum.crypto.hash.*;
...
public class Checksum {
    /* exit codes
       ...
       2 input error (checkfile)
       3 input error (file system)
     */
    ...
    private final static String METAINFO = CHECKSUM + ": Meta-Info: ";
    private final static String COMMENT = CHECKSUM + ": Comment: ";
    ...
    private boolean _f = false, _x = false, _X = false, _r = false, _t = false,
            _m = false, _p = false, _y = false,
                                         _l = false, _d = false, _S = false, _e = false,
            _F = false, _alternate = false, _P = false;


    /** Checksum's main method
    ...
    /** get Checksum's program version
    ...
    public void printHelpShort() {
        ...
        System.out.println(" java " + CHECKSUM + " -h en");
        System.out.println("\n Fuer weitere Informationen bitte eingeben:");
        System.out.println(" java " + CHECKSUM + " -h de\n");
        System.exit(0);
    }
        ...
        if (file.isDirectory()) {
            ...
            if (!_d || (_d && !Service.isSymbolicLink(file))) {
                ...
                if (list == null) {
                    ...
                    // this strange behaviour was only detected on Windows with a folder called
                    // "C:\System Volume Information"
                    System.err.println(
                            "Jacksum: Can't access file system folder \"" +
                            file + "\"");
                } else {
                    if (!_p && !_S) {
                        if (_P) {
                            System.out.println("\n" +
                                    ...
                                    separatorChar, fileseparatorChar) + ":");
                        } else {
                            System.out.println("\n" + file + ":");
                        }
                    }
    ...
    public void oneDir(String dirItem) {
        ...
        if (file.isDirectory()) {
            ...
            if (list == null) {
                System.err.println(
                        "Jacksum: Can't access file system folder \"" + file +
                        "\"");
            } else {
                Arrays.sort(list, String.CASE_INSENSITIVE_ORDER);
     ...
     * @param checkFile the filename containing checksums
     * @throws FileNotFoundException if file is not there
     * @throws IOException during an IO error
     */
        ...
        try {
            ...
            if ((thisLine = br.readLine()) != null) {
                if (thisLine.startsWith(METAINFO)) {
                    st = new StringTokenizer(thisLine.substring(METAINFO.length(),
                            thisLine.length()), ";");
                    while (st.hasMoreTokens()) {
                        stsub = new StringTokenizer(st.nextToken(), "=");
                        props.setProperty(stsub.nextToken(),
                                          (stsub.hasMoreTokens() ?
                                           stsub.nextToken() : ""));
                    }
                    // version check
                    if (new Version(props.getProperty("version")).compareTo(new
                            Version(VERSION)) > 0) {
                        System.err.println("The file called " + checkFile +
                    ...
                    System.err.println("File does not contain meta information. Create meta info with option -m.\nExit.");
                    System.exit(2);
                }
            } else {
            ...
            try {
                checksum = ChecksumAPI.getChecksumInstance(props.getProperty(
            ...
            if (props.getProperty("tformat").equals("")) {
                ...
                if (props.getProperty("tformat").equals("null")) { // bugfix for sourceforge-bug #948070 (problem with files generated with 1.4.0)
                    System.err.println(
                            "Jacksum: Can't determine timeformat (tformat=null), trying default (tformat=" +
                    ...
                    ignoretokens +=
                            GeneralString.countChar(props.getProperty("tformat"),
                            '\t');
                }
            }
            ...
            while ((thisLine = br.readLine()) != null) {
                if ((!thisLine.equals("")) && // ignore empty lines
                    (!thisLine.startsWith(COMMENT)) // ignore Jacksum comment lines
                        ) {
                    if (thisLine.startsWith(CHECKSUM)) {
                        System.err.println(CHECKSUM +
                                           ": Ignoring unknown directive.");
                    } else {

                        if ((_r && !_p) && thisLine.endsWith(":")) { // it is a folder
    ...
    private void skipOkFiles(String filename, String thisLine, int skip) throws
            IOException {
        boolean out = false;
    ...
    private String whatChanged(String filename, String thisLine, int skip) throws
            IOException {
        if (!(new File(filename).exists())) {
            return ("[REMOVED] ");
        } else {
    ...
    private void expectation(AbstractChecksum checksum, String expected) {
        String value = ((_x || _X) ? checksum.getHexValue() :
    ...
    private String parseFilename(String thisLine, int ignoretokens) throws
    ...
    public Checksum(String[] args) {
        com.progdan.checksum.util.GeneralProgram.requiresMinimumJavaVersion(
                "1.3.1");
        boolean stdin = false, _s = false, _D = false, _q = false, _c = false,
                _b = false;
            ...
            while (firstfile < args.length && args[firstfile].startsWith("-")) {
                ...
                if (arg.equals("-a")) {
                    if (firstfile < args.length) {
                        ...
                        System.err.println(
                                "Option -a requires an algorithm. Use -h for help. Exit.");
                        System.exit(1);
                    }
                    ...
                    if (firstfile < args.length) {
                        _s = true;
                        arg = args[firstfile++];
                        separator = com.progdan.checksum.util.GeneralString.
                                    translateEscapeSequences(arg);
                    } else {
                        System.err.println(
                                "Option -s requires a separator string. Use -h for help. Exit.");
                        System.exit(1);
                    }
                    ...
                    if (firstfile < args.length) {
                        ...
                        System.err.println(
                                "Option -q requires a hex sequence argument");
                        System.exit(1);
                    }
                } else if (arg.equals("-P")) {
                    if (firstfile < args.length) {
                        ...
                        if (arg.length() != 1) {
                            System.out.println(
                                    "Option -P requires exactly one character");
                            System.exit(1);
                        } else {
                            if (arg.charAt(0) == '/' || arg.charAt(0) == '\\') {
                                fileseparatorChar = arg.charAt(0);
                            } else {
                                System.err.println("Option -P requires / or \\");
                                System.exit(1);
                            }
                        }
                    } else {
                    ...
                    if (firstfile < args.length) {
                        ...
                        System.err.println(
                                "Option -c requires a filename parameter");
                        System.exit(1);
                    }
                    ...
                    if (firstfile < args.length) {
                        ...
                        System.err.println(
                                "Option -t requires a format string. Use -h for help. Exit.");
                        System.exit(1);
                    }
        ...
        try {
            checksum = ChecksumAPI.getChecksumInstance(checksumArg, _alternate);
        } catch (NoSuchAlgorithmException nsae) {
        ...
        if (_S && _m) {
            System.out.println(
                    "Jacksum: -S and -m can't go together, it is not supported");
            System.exit(1);
        }
        ...
        if (_m) {
            ...
            if (_t) {
                if (timestampFormat.indexOf(";") > -1) {
                    System.err.println(
                            "Option -t contains a semicolon. This is not supported with -m.");
                    System.exit(1);
                }
            }
            _F = false;
        ...
        if (_q) { // quick sequence and quit
            ...
            if (_t) {
                System.err.println(
                        "Jacksum: Option -t will be ignored, because option -q is used.");
                _t = false;
                checksum.setTimestampFormat(null);
            }
            ...
            if (seqlower.startsWith("dec:")) {
                ...
                if ((sequence.length() % 2) == 1) {
                    System.err.println(
                            "An even number of nibbles was expected.\nExit.");
                    System.exit(1);
                }
                try {
            ...
            if (_e) {
                ...
                System.out.println(_F ? checksum.format(format) :
                                   checksum.toString());
            }
            ...
            if (_c) {
                ...
                if (!f.exists()) {
                    ...
                    if (f.isDirectory()) {
                        System.err.println(
                                "Parameter is a directory, but a filename was expected. Exit.");
                        System.exit(1);
                    } else {
                        try {
                            checkFile(checkfile);
        ...
        if (_r || _D) {
            ...
            if (args.length == firstfile) {
                dir = ".";
            } else {
                System.err.println(
                        "Too many parameters. One directory was expeced. Exit.");
                System.exit(1);
            }
            File f = new File(dir);
            if (!f.exists()) {
                ...
                if (f.isDirectory()) {
                    ...
                    System.err.println(
                            "Parameter is a file, but a directory was expected. Exit.");
                    System.exit(1);
                }
            }
        } else {
            ...
            if (stdin || (firstfile == args.length)) { // no file parameter
                ...
                if (_t) {
                    System.err.println(
                            "Jacksum: Option -t will be ignored, because standard input is used.");
                    _t = false;
                    checksum.setTimestampFormat(null);
                }
                checksum.setFilename("");
