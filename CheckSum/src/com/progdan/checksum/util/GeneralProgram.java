 ...
 *
 *****************************************************************************/
package com.progdan.checksum.util;
...
public class GeneralProgram {
  ...
  public final static void requiresMinimumJavaVersion(final String version)
  {
    try {
      ...
      if (
		    (
			  System.getProperty("java.vm.vendor").startsWith("Sun Microsystems") ||
              System.getProperty("java.vm.vendor").startsWith("IBM Corporation") ||
              System.getProperty("java.vm.vendor").startsWith("Hewlett-Packard")
            )
            && (ver.compareTo(version) < 0)
         )
      {
         System.out.println("ERROR: a newer Java VM is required.\nVersion of your Java VM: "+ver+
                             "\nRequired minimum version: "+ version);
         System.exit(1);
      }
    } catch (Throwable t) {
